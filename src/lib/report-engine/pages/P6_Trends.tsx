              ...
              <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 14 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {data.diagnosticInsights.slice(0, 2).map((ins, i) => (
                    <div key={i} className="rpt-ai-block" style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontSize: 10.5, fontWeight: 750, color: "var(--rpt-brand-dark)", marginBottom: 4 }}>
                          {ins.title}
                        </div>
                        <p style={{ fontSize: 9.2, color: "var(--rpt-text-muted)", lineHeight: 1.45, margin: 0 }}>
                          {ins.summary}
                        </p>
                      </div>
                    </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {data.topFindings.slice(0, 4).map((f, i) => (
                  <div key={i} className="rpt-card-sm" style={{ padding: 10 }}>
                    ...
                    <p style={{ fontSize: 9.8, color: "var(--rpt-text-muted)", lineHeight: 1.4, margin: 0 }}>
                      {f}
                    </p>
                  </div>
                ))}
        ...
        {primary && (
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 10, color: "var(--rpt-text-muted)", lineHeight: 1.5, margin: 0 }}>
                Analysis detects a total growth shift of {primary.totalGrowthPct.toFixed(1)}% across monitored periods. The peak level was reached in {primary.peakPeriod} and the lowest inflection point was registered in {primary.troughPeriod}.
              </p>
            </div>
            <div style={{ borderLeft: "1px solid var(--rpt-border)", paddingLeft: 20, display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 9, color: "var(--rpt-text-muted)" }}>Seasonality:</span>
                <ReportBadge label={primary.seasonalityDetected ? "Detected" : "None"} variant={primary.seasonalityDetected ? "success" : "neutral"} />
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {data.diagnosticInsights.slice(0, 3).map((ins) => (
                  <div key={ins.id} className="rpt-ai-block" style={{ padding: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                      <div style={{ fontSize: 10, fontWeight: 750, color: "var(--rpt-brand-dark)" }}>{ins.title}</div>
            ...
            <ReportSection title="Operational Action Notes">
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {data.topFindings.slice(0, 3).map((finding, idx) => (
                  ...
                  >
                    <div style={{ fontSize: 9, fontWeight: 800, color: "var(--rpt-brand)", textTransform: "uppercase", marginBottom: 3 }}>
                      Trend Signal {idx + 1}
                    </div>
                    <p style={{ fontSize: 9.5, color: "var(--rpt-text-muted)", lineHeight: 1.4, margin: 0 }}>
                      {finding}
                    </p>
                  </div>
                ))}
