...
const DUMMY_AUDIT_LOGS = [
  ...
  { action: "Database initialized (SQLite)", time: "5 mins ago", user: "System" },
  { action: "Collaborator seat assigned: FullStacker", time: "10 mins ago", user: "AyushPanthri" },
  { action: "Backend repository sync complete", time: "30 mins ago", user: "Teammate" },
  { action: "FastAPI endpoints mapped: /analyze", time: "1 hour ago", user: "Teammate" },
  { action: "Role permissions verification passed", time: "2 hours ago", user: "Security" },
];
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between">
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 w-full flex-grow space-y-8">
        ...
        <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-primary/15 via-info/5 to-transparent p-8 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-3 max-w-2xl">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed sm:text-base">
              Welcome to the **InsightForge AI Platform**. The analytical workspace is fully operational.
              You have access permissions to upload datasets, analyze quality issues, customize
              scenario forecasting modules, and run explainable AI queries.
            </p>
            <div className="pt-2">
              <Link
                to="/workspace"
              ...
              >
                <Upload className="h-4 w-4" /> Launch Analytics Workspace <ChevronRight className="h-4 w-4" />
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Server Health", value: "ONLINE", icon: Activity, detail: "Pulsing green logic", color: "text-success" },
            { label: "Database Engine", value: "SQLite (Dev)", icon: Database, detail: "Local workspace DB", color: "text-primary" },
            { label: "API Latency", value: "14 ms", icon: Clock, detail: "Secure backend routing", color: "text-info" },
            { label: "Seat Occupancy", value: "4 Seats Active", icon: Users, detail: "Team collaborator limit", color: "text-warning" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
          ...
          <div className="lg:col-span-2 rounded-2xl border border-white/5 bg-surface/20 p-6 flex flex-col justify-between space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Quickly parse a new dataset. Staged files are processed under sandbox security rules.
              </p>
            </div>
            ...
            <div className="rounded-xl border-2 border-dashed border-white/8 bg-background/30 p-8 text-center flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-muted-foreground max-w-xs">
                Launch the workspace to upload datasets and generate four levels of evidence-backed reports.
              </p>
              <Link
                to="/workspace"
          ...
          <div className="rounded-2xl border border-white/5 bg-surface/30 p-6 space-y-5">
            ...
            <div className="space-y-4 max-h-[240px] overflow-y-auto scrollbar-thin pr-1">
              {DUMMY_AUDIT_LOGS.map((log, index) => (
                <div key={index} className="flex items-start gap-3 border-l-2 border-white/5 pl-3 py-0.5 hover:border-primary/45 transition-colors">
                  <div className="flex-grow space-y-1">
                    <p className="text-xs font-semibold leading-relaxed text-foreground/90">{log.action}</p>
