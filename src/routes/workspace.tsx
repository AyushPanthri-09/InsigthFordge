...
import { downloadExecutivePdf } from "@/lib/pdf-report";
...
export const Route = createFileRoute("/workspace")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Upload a dataset and let InsightForge AI run the complete analyst workflow." },
    ],
  }),
  component: Workspace,
});

type Tab = "overview" | "understanding" | "cleaning" | "eda" | "analytics" | "report";
...
function Workspace() {
  ...
  const reset = () => { setAnalysis(null); setSteps([]); setCurrent(""); };
        ...
        {!analysis ? (
          <div className="mx-auto grid max-w-4xl gap-6 lg:grid-cols-[1fr_360px]">
            <div className="space-y-4">
              <div>
                <h1 className="font-display text-3xl font-semibold tracking-tight">Begin a new analysis</h1>
                <p className="mt-1.5 text-sm text-muted-foreground">
                  Drop a CSV or XLSX. InsightForge AI will understand its business context, justify every cleaning decision, run EDA, and produce four levels of analytics.
                </p>
              </div>
              ...
              <div className="card-elevated p-4 text-xs text-muted-foreground">
                ...
                Your file is processed in your session. Sensitive data never leaves the analysis context.
              </div>
            </div>
...
function Dashboard({
  analysis, tab, onTab, notes, onNotesChange,
}: {
  ...
  const tabs: Array<{ id: Tab; label: string; count?: number }> = useMemo(() => [
    ...
    { id: "cleaning", label: "Cleaning", count: analysis.cleaning.issues.length },
    { id: "eda", label: "EDA", count: analysis.eda.charts.length },
    { id: "analytics", label: "Analytics",
      count: analysis.analytics.descriptive.length + analysis.analytics.diagnostic.length + analysis.analytics.predictive.length + analysis.analytics.prescriptive.length },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight">{analysis.dataset.fileName}</h1>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {analysis.dataset.rowCount.toLocaleString()} rows × {analysis.dataset.columnCount} columns ·
            Domain: <span className="text-foreground">{analysis.understanding.domain}</span>
          </p>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {tab === "overview" && <OverviewTab analysis={analysis} notes={notes} onNotesChange={onNotesChange} />}
...
function OverviewTab({
  analysis, notes, onNotesChange,
}: { analysis: FullAnalysis; notes: string; onNotesChange: (v: string) => void }) {
  const topInsights = [
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        <HealthScoreCard score={analysis.analytics.businessHealthScore} summary={analysis.analytics.executiveSummary} />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {analysis.eda.kpis.slice(0, 8).map((k) => <KpiCard key={k.id} kpi={k} />)}
        </div>
        ...
        {topInsights.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">Click to expand evidence</span>
            </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {analysis.eda.kpis.map((k) => <KpiCard key={k.id} kpi={k} />)}
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        {analysis.eda.charts.map((c) => <ChartCard key={c.id} spec={c} />)}
      </div>
      {analysis.eda.correlations.length > 0 && (
        <div className="card-elevated p-5">
          <h3 className="text-sm font-semibold">Top Correlations</h3>
          <p className="text-xs text-muted-foreground">Pearson correlations between numeric measures.</p>
            ...
            {analysis.eda.correlations.map((c, i) => (
              <div key={i} className="flex items-center justify-between rounded-lg border border-border/60 bg-background/40 p-3">
                <div className="text-sm">
                  <span className="font-mono text-xs text-muted-foreground">{c.a}</span>
                  <span className="mx-2 text-muted-foreground">↔</span>
                  <span className="font-mono text-xs text-muted-foreground">{c.b}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${c.strength === "strong" ? "bg-success/15 text-success" : c.strength === "moderate" ? "bg-info/15 text-info" : "bg-muted text-muted-foreground"}`}>
                    {c.strength}
                  </span>
                  <span className="font-mono text-sm tabular-nums">{c.r.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
...
function AnalyticsTab({ analysis }: { analysis: FullAnalysis }) {
  const sections: Array<{ id: string; title: string; subtitle: string; items: typeof analysis.analytics.descriptive }> = [
    { id: "desc", title: "Descriptive", subtitle: "What happened — observed from the data.", items: analysis.analytics.descriptive },
    { id: "diag", title: "Diagnostic", subtitle: "Why it happened — hypotheses tested against evidence.", items: analysis.analytics.diagnostic },
    { id: "pred", title: "Predictive", subtitle: "What may happen — with explicit assumptions and limitations.", items: analysis.analytics.predictive },
    { id: "presc", title: "Prescriptive", subtitle: "What to do — prioritized, justified recommendations.", items: analysis.analytics.prescriptive },
  ];
  return (
      ...
      {sections.map((s) => (
        <section key={s.id}>
          <div className="mb-3 flex items-baseline justify-between gap-3">
            ...
            <span className="text-xs text-muted-foreground">{s.items.length} insight{s.items.length === 1 ? "" : "s"}</span>
          </div>
          {s.items.length === 0 ? (
            <div className="card-elevated p-6 text-center text-sm text-muted-foreground">No {s.title.toLowerCase()} insights generated for this dataset.</div>
          ) : (
            <div className="space-y-3">
              {s.items.map((i) => <InsightCard key={i.id} insight={i} />)}
            </div>
          )}
        </section>
      ))}
...
function ReportTab({ analysis }: { analysis: FullAnalysis }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast.error("PDF generation failed", { description: e instanceof Error ? e.message : "Unknown error" });
    } finally {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Live preview of the full 9-page PDF report</p>
        </div>
        <button
