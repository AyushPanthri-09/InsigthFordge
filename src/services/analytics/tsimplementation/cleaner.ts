...
export function detectIssues(
    ...
    issues.push({
      ...
      possibleCauses: [
        ...
        },
        {
          ...
          evidence: "Higher duplication rates sometimes indicate event-driven systems where the same state is recorded multiple times.",
        },
        {
  ...
  for (const p of profiles) {
    ...
    const nullDecision = buildDAIENullDecision(p, nullPct, category, schemaRole, profiles, intelligence);
  ...
  for (const p of profiles) {
    ...
    if (hasStripWhitespace) {
      issues.push({
        ...
        description: "String values contain unnecessary whitespace that can break grouping, matching, and validation.",
    ...
    const hasCaseInconsistency = Array.from(lowerCaseVariants.values()).some((variants) => variants.size > 1);
    if (hasCaseInconsistency) {
      issues.push({
        ...
        description: "Values differ only by letter case, which can create duplicate categories and inconsistent filters.",
    ...
    if (convertibleCount >= Math.max(2, Math.floor(stringSamples.length * 0.3))) {
      issues.push({
        ...
        description: "String values appear to represent numbers, dates, booleans, or percentages. Converting them preserves analytical semantics.",
    ...
    const phoneSamples = stringSamples.filter((s) => /phone|tel|contact/i.test(p.name) || shouldNormalizePhone(s));
    if (phoneSamples.length > 0 && phoneSamples.some((s) => shouldNormalizePhone(s))) {
      issues.push({
        ...
        description: "Phone values use inconsistent formatting. Normalizing them improves matching and lookup reliability.",
  ...
  if (rows.length > 5) {
    ...
    if (firstFewMostlyNull >= 2) {
      issues.push({
        ...
        confidence: 0.65,
        businessImpact: "Prevents skewed type inference and false outlier detection caused by header text appearing in data rows.",
        requiresApproval: true,
        possibleCauses: [
          { cause: "Excel/BI tool export with title or subtitle rows", plausibility: 0.7, evidence: "Leading rows have >60% null values." },
          { cause: "Merged cells in source spreadsheet creating blank rows", plausibility: 0.25, evidence: "Pattern matches typical spreadsheet header formatting." },
        ],
        alternativeInterpretations: [
  ...
  for (const p of profiles) {
    ...
    if (sampleNums.length > 0 && sampleStrs.length > 0) {
      issues.push({
        ...
        requiresApproval: true,
        possibleCauses: [
          { cause: "N/A, NULL, or placeholder strings mixed with numeric values", plausibility: 0.65, evidence: `Sample text values: ${sampleStrs.slice(0, 3).join(", ")}.` },
          { cause: "Column stores both a code and a description (e.g., '10 - Cash')", plausibility: 0.25, evidence: "Composite value pattern common in legacy exports." },
        ],
        alternativeInterpretations: [
...
function buildDAIENullDecision(
  ...
  const hasDependency =
    ...
    schemaRole === "dimension_key" ||
    schemaRole === "date_key" ||
    Boolean(intelligence?.[profile.name]?.isKpiCandidate || intelligence?.[profile.name]?.isForecastCandidate);
  const sparseLikelyMeaningful =
    nullPct > 0.5 &&
    (highBusinessValue || category === "status_flag" || /fraud|risk|refund|return|cancel|churn|claim|error/i.test(profile.name));
  ...
  if (nullPct <= 0.25 && profile.inferredRole === "measure") {
    ...
    justification =
      `Use safe retention plus an explicit missingness indicator for "${profile.name}". Numeric imputation may be considered later only if distribution shape and correlated predictors support it.`;
    rejectedOptions.push("Blind mean/median imputation rejected because dependency and missingness mechanism are not proven.");
    rejectedOptions.push("Drop rejected because missingness is not high enough and measure columns may carry analytical signal.");
  } else if (sparseLikelyMeaningful || hasDependency) {
    finalDecision = "transform";
    justification =
      `Retain "${profile.name}" and encode missingness as information. Dependency/business layers override deletion: the column may be structurally required or represent rare but valuable events.`;
    rejectedOptions.push("Drop rejected because dependency or high-business-value evidence overrides missing-percentage heuristics.");
    rejectedOptions.push("Mode/median imputation rejected because missingness may be meaningful rather than noise.");
  } else if (nullPct > 0.9 && !highBusinessValue && !hasDependency && profile.uniqueCount <= 1) {
    finalDecision = "flag_for_review";
    justification =
      `"${profile.name}" is a drop candidate, but deletion still requires human/source-system validation because missingness alone is not sufficient evidence.`;
    rejectedOptions.push("Automatic drop rejected: the system cannot prove the column is not a recently added, optional, or rare-event field.");
  } else {
    finalDecision = "flag_for_review";
    justification =
      `Retain pending investigation. Test whether nulls cluster by time, source system, segment, or entity before choosing imputation, transformation, or deletion.`;
    rejectedOptions.push("Single-threshold deletion rejected.");
    rejectedOptions.push("Default imputation rejected until distribution and dependency structure are validated.");
  }
  ...
  return {
    ...
    justification,
    riskAssessment:
      `Wrong handling can bias aggregates, erase rare signals, or break downstream charts/KPIs. Safest current path is ${finalDecision === "transform" ? "safe retention with explicit missingness handling" : "review before mutation"}.`,
    rejectedOptions,
  };
}
...
function classifyDAIERole(
  ...
  if (schemaRole === "primary_key" || profile.inferredRole === "key") return "Primary Key / Identifier";
  if (schemaRole === "dimension_key") return "Identifier / Foreign-key feature";
  if (schemaRole === "fact_measure" || profile.inferredRole === "measure") return "Feature / numeric measure";
  if (schemaRole === "date_key" || profile.inferredRole === "date") return "Time-series continuity feature";
  if (category === "descriptor") return "Descriptive / segmentation feature";
  if (profile.uniqueCount <= 1 && profile.nonNullCount <= 1) return "Noise / low-information candidate";
...
function describeDAIEDependency(
  ...
  const siblings = profiles
    .filter((p) => p.name !== profile.name && siblingPrefix.length >= 3 && p.name.toLowerCase().includes(siblingPrefix))
    .map((p) => p.name)
    .slice(0, 4);
  const intel = intelligence?.[profile.name];

  if (intel?.isForecastCandidate) return "Required for time-series continuity or forecasting; dropping could break temporal analysis.";
  if (intel?.isKpiCandidate) return "Feeds KPI computation; missingness affects headline metrics.";
  if (siblings.length > 0) return `Potential parent/related field for ${siblings.join(", ")}; dependency integrity should be checked before mutation.`;
  if (profile.inferredRole === "dimension") return "Used for segmentation, grouping, or aggregation when cardinality is suitable.";
...
function describeDAIEBusinessImportance(
  ...
  if (category === "financial_metric") return "High: financial/revenue signal; nulls may affect KPIs and cannot be treated as zero without validation.";
  if (category === "entity_key" || schemaRole === "primary_key" || schemaRole === "dimension_key") return "High: identity/join/compliance relevance; missingness may represent anonymous or unmatched entities.";
  if (category === "time_dimension") return "High: event lifecycle and period analysis depend on temporal completeness.";
  if (category === "geo_dimension") return "Medium/high: affects regional segmentation and compliance/privacy interpretation.";
  if (/fraud|risk|refund|return|cancel|churn|claim/i.test(profile.name)) return "High: rare-event or risk signal; sparsity may be meaningful.";
...
function describeDAIEStatisticalProfile(profile: ColumnProfile, nullPct: number): string {
  const missingness =
    nullPct > 0.8 ? "likely MNAR/MAR candidate until proven otherwise"
    : nullPct > 0.3 ? "MAR/MNAR should be tested by segment and time"
  ...
  const skewHint = Math.abs(profile.stats.mean - profile.stats.median) > Math.max(1e-9, Math.abs(profile.stats.stdev) * 0.25)
...
function describeDAIEDataQuality(profile: ColumnProfile, nullPct: number): string {
  ...
  if (profile.stats?.outlierCount) issues.push(`${profile.stats.outlierCount} statistical outlier candidates`);
  if (profile.inferredType === "unknown") issues.push("unstable or unknown inferred type");
  if (profile.sampleValues.some((v) => typeof v === "string" && v.trim() === "")) issues.push("empty-string missing values present in samples");
...
function buildImputationOptionReasoning(
  ...
  if (profile.inferredRole === "date" || category === "time_dimension" || schemaRole === "date_key") {
  ...
  if (profile.inferredRole === "measure" || profile.stats) {
    const symmetric = profile.stats
      ? Math.abs(profile.stats.mean - profile.stats.median) <= Math.max(1e-9, Math.abs(profile.stats.stdev) * 0.25)
      : false;
...
function buildNullReasoning(
  ...
  if (category === "financial_metric") {
    possibleCauses.push(
      { cause: "Transaction did not occur (e.g., cancelled order — no payment amount)", plausibility: 0.5, evidence: `Financial columns are often null when the transaction that populates them did not complete.` },
      { cause: "Data migration gap — historical records pre-date this field", plausibility: 0.3, evidence: `${pctStr} null rate may indicate this column was added after the dataset began.` },
      { cause: "Data entry error or upstream pipeline failure", plausibility: 0.2, evidence: "Random null distribution without business logic." },
    );
    ...
    possibleCauses.push(
      { cause: "Event not yet occurred (future-dated records)", plausibility: 0.55, evidence: "Date columns are commonly null when the event they record has not happened yet." },
      { cause: "Optional event that only applies to a subset of records", plausibility: 0.35, evidence: `${pctStr} null rate suggests this date only applies to a subset.` },
    );
    alternativeInterpretations.push(
    ...
    possibleCauses.push(
      { cause: "Online or digital channel where geography is not captured", plausibility: 0.45, evidence: "Geographic fields are often optional in digital-first datasets." },
      { cause: "Data privacy masking or GDPR compliance", plausibility: 0.3, evidence: "Geographic granularity is frequently suppressed for privacy." },
    );
    alternativeInterpretations.push("Null geography may represent 'online/unknown' — consider encoding as 'Unknown' rather than dropping.");
  } else if (category === "entity_key") {
    possibleCauses.push(
      { cause: "Optional relationship — not all records are linked to this entity", plausibility: 0.6, evidence: "FK columns are null when the relationship is optional (outer join)." },
      { cause: "Guest or anonymous records that have no entity link", plausibility: 0.3, evidence: "Common in e-commerce (guest checkout) and CRM (anonymous leads)." },
    );
    alternativeInterpretations.push(`Null "${colName}" may represent anonymous or unregistered entities — preserve rather than drop.`);
  } else {
    // Generic causes
    possibleCauses.push(
      { cause: "Optional field not filled by all data sources", plausibility: 0.5, evidence: `${pctStr} null rate suggests optional field.` },
      { cause: "Data pipeline gap — field not populated by all upstream systems", plausibility: 0.3, evidence: "Multi-source datasets commonly have partial field coverage." },
      { cause: "Field added retroactively — historical records lack this data", plausibility: 0.2, evidence: "Common when schema evolves over time." },
    );
    alternativeInterpretations.push(
  ...
  return { reasoning, possibleCauses, alternativeInterpretations, action, severity, confidence };
}
...
function buildOutlierReasoning(
  ...
  if (category === "financial_metric") {
    possibleCauses.push(
      { cause: "Bulk/corporate purchase or enterprise customer order", plausibility: 0.45, evidence: `Extreme values in financial columns often reflect large B2B transactions rather than errors.` },
      { cause: "Seasonal or promotional spike (holiday sale, flash deal)", plausibility: 0.35, evidence: "Financial outliers frequently align with peak seasons or campaign periods." },
      { cause: "Data entry error — wrong unit (e.g., paise instead of rupees, cents instead of dollars)", plausibility: 0.15, evidence: "Unit mismatch is a common data quality issue in financial columns." },
      { cause: "Fraud or anomalous transaction", plausibility: 0.05, evidence: "Statistical outliers are a primary fraud detection signal — never auto-remove." },
    );
    alternativeInterpretations.push(
    ...
    possibleCauses.push(
      { cause: "Bulk order or warehouse replenishment transaction", plausibility: 0.5, evidence: "Quantity outliers in operational data frequently reflect planned bulk operations." },
      { cause: "Data entry error (e.g., extra zero added)", plausibility: 0.3, evidence: "Quantity fields are prone to digit-doubling errors in manual entry." },
      { cause: "System-generated test or initialisation record", plausibility: 0.2, evidence: "Test records are sometimes not filtered from production exports." },
    );
    alternativeInterpretations.push(
    ...
    possibleCauses.push(
      { cause: "Genuine extreme event (rare but real business occurrence)", plausibility: 0.5, evidence: "IQR-based outlier detection is sensitive; not all flagged values are errors." },
      { cause: "Data quality issue — measurement or entry error", plausibility: 0.3, evidence: "Statistical extremes warrant manual investigation." },
      { cause: "Different population mixed into this dataset (e.g., B2B + B2C)", plausibility: 0.2, evidence: "Multi-segment datasets often show bimodal distributions that appear as outliers." },
    );
    alternativeInterpretations.push(
