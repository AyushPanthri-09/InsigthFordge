...
export function calculateDataQuality(
  ...
  if (rows.length === 0 || profiles.length === 0) {
    return {
      overallScore: 0,
      categoryScores: { completeness: 0, uniqueness: 0, consistency: 0, validity: 0, typeAlignment: 0 },
  ...
  if (completeness < 95) {
    recommendations.push(`Fill missing values (completeness: ${completeness.toFixed(1)}%). Consider inputing missing fields or reviewing import pipelines.`);
  }

  // 2. Uniqueness Score (Duplicates)
  const uniqueness = rows.length > 0 ? ((rows.length - duplicateRowCount) / rows.length) * 100 : 100;
  if (duplicateRowCount > 0) {
    recommendations.push(`Remove ${duplicateRowCount} duplicate rows to ensure exact uniqueness representation.`);
  }
  ...
  const consistency = totalNumericCells > 0 ? Math.max(0, (1 - totalOutlierCount / totalNumericCells) * 100) : 100;
  if (totalOutlierCount > 0) {
    recommendations.push(`Investigate ${totalOutlierCount} statistical outliers found in numerical measures.`);
  }
  ...
  const validityAnomalies = anomalies.filter(a => a.type === "negative_inventory" || a.type === "negative_revenue" || a.type === "duplicate_transaction");
  const validityScore = Math.max(0, 100 - (validityAnomalies.length * 10));

  if (validityAnomalies.length > 0) {
    recommendations.push(`Resolve ${validityAnomalies.length} validity warnings including impossible negative inventory or revenue values.`);
  }
  ...
  if (mismatchCount > 0) {
    recommendations.push(`Re-align data types for ${mismatchCount} columns containing non-numeric values in numeric measures.`);
  }
