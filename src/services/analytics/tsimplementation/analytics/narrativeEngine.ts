...
export function generateSCQANarrative(
  ...
  if (primaryKpi) {
    ...
    situation +=
      "Standard operational variables and metrics were analyzed to assess current performance parameters.";
  }
  ...
  if (critAnomalies.length > 0) {
    ...
    complication +=
      "Our diagnostic checks flagged potential inefficiencies and variance within key measures that limit top-line execution. ";
  }
  ...
  if (correlations.length > 0) {
    ...
    insight +=
      "the relationships between dimension categories and measures indicate that performance is heavily segmented. ";
  }
  ...
  if (recommendations.length > 0) {
    ...
    expectedOutcome +=
      "restore operational stability, correct accounting discrepancies, and enhance forecasting accuracy. ";
  }
  expectedOutcome +=
    "Failure to act poses significant risks, including operational write-offs and customer churn.";
