  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Total Row Count", val: dataset?.rowCount?.toLocaleString() || "0", icon: Database },
          { label: "Total Column Count", val: dataset?.columnCount?.toLocaleString() || "0", icon: Table },
          { label: "Quality Health Score", val: `${understanding?.domainConfidence ? Math.round(understanding.domainConfidence * 100) : 95}%`, icon: BarChart },
        ].map((kpi) => (
          <div key={kpi.label} className="card-elevated p-5 flex items-center justify-between border border-white/5 hover:border-white/10 transition-all">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="card-elevated p-6 space-y-4">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1 scrollbar-thin">
            {understanding?.columnProfiles?.map((col: any) => (
              <div key={col.name} className="rounded-xl border border-white/5 bg-background/40 p-3.5 flex items-center justify-between">
                ...
                <div className="text-right space-y-0.5">
                  <div className="text-[10px] font-mono text-primary font-bold">{col.inferredType}</div>
                  <div className="text-[9px] text-muted-foreground/60">{col.nullCount > 0 ? `${Math.round((col.nullCount/dataset.rowCount)*100)}% null` : "0% null"}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 card-elevated p-6 space-y-5 flex flex-col justify-between">
            ...
            <table className="w-full text-[11px] text-left border-collapse">
              ...
              <tbody>
                {filteredRows.slice(0, 15).map((row: any, rIdx: number) => (
                  <tr key={rIdx} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    {dataset.columns.map((col: string) => (
                      <td key={col} className="px-4 py-2.5 truncate max-w-[150px] font-medium text-foreground/80">
                ...
                {filteredRows.length === 0 && (
                  <tr>
                    <td colSpan={dataset?.columns?.length || 1} className="text-center py-8 text-muted-foreground">
          ...
          <div className="flex items-center justify-between text-[10px] text-muted-foreground/60 mt-1">
            <span>Showing top {Math.min(15, filteredRows.length)} of {filteredRows.length} rows</span>
