...
export function PredictiveSandbox({ dataset }: { dataset: any }) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let month = 1; month <= 6; month++) {
      ...
      // Calculations
      const activeCustomers = Math.round((baseVolume * volumeMultiplierFromDiscount + volumeFromAdSpend) * teamEfficiency * (1 + (month * 0.05)));
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <div className="card-elevated p-6 space-y-6 flex flex-col justify-between">
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Adjust internal business variables to run real-time math projections on your dataset matrix.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-white/5 bg-background/40 p-3.5 text-[11px] text-muted-foreground flex items-start gap-2">
          ...
          <p className="leading-relaxed">
            Mathematical parameters are computed client-side using deterministic polynomial formulas relative to dataset size.
          </p>
        </div>
      </div>
      ...
      <div className="lg:col-span-2 space-y-6 flex flex-col justify-between">
        ...
        <div className={`rounded-xl border p-4 flex items-start gap-3 transition-colors duration-300 ${aiAdvice.status}`}>
        ...
        <div className="card-elevated p-6 flex-grow flex flex-col justify-between min-h-[300px]">
          <div className="flex items-center justify-between mb-4">
            <div className="space-y-0.5">
              ...
              <p className="text-[11px] text-muted-foreground">Deterministic 6-month simulation path</p>
            </div>
          </div>
          ...
          <div className="w-full h-[240px] flex-grow">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis stroke="oklch(1 0 0 / 40%)" fontSize={11} tickLine={false} tickFormatter={(v) => `$${(v/1000)}k`} />
                <Tooltip 
                  contentStyle={{
                  ...
                  labelStyle={{ fontSize: 11, fontWeight: "bold", color: "oklch(0.85 0.19 95)" }}
                />
                <Legend wrapperStyle={{ fontSize: 11, paddingTop: 10 }} />
                <Area type="monotone" dataKey="Net Revenue ($)" stroke="oklch(0.85 0.19 95)" strokeWidth={2} fillOpacity={1} fill="url(#colorRev)" />
                <Area type="monotone" dataKey="Net Profit ($)" stroke="oklch(0.80 0.14 195)" strokeWidth={2} fillOpacity={1} fill="url(#colorProfit)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Margin", val: `${chartData[5]["Margin (%)"]}%`, unit: "Peak month 6", color: "text-primary" },
            { label: "Rev Gain", val: `$${(chartData[5]["Net Revenue ($)"]/1000).toFixed(0)}k`, unit: "Month 6 target", color: "text-info" },
            { label: "Active Cust", val: chartData[5]["Active Customers"].toLocaleString(), unit: "Forecast scale", color: "text-success" },
          ].map((kpi) => (
            <div key={kpi.label} className="rounded-xl border border-white/5 bg-surface/30 p-3.5 text-center">
